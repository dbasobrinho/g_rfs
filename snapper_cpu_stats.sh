...
# Package       :   BU Plugins
# Description   :   GUINA plugin (script) to check cpu utilization  
#                   statistics.
...
#
# USAGE  : ./snapper_cpu_stats.sh [-w <user,system,iowait>] [-c <user,system,iowait>] ( [ -i <intervals in second> ] [ -n <report number> ])
#           
# Exemple: ./snapper_cpu_stats.sh 
#          ./snapper_cpu_stats.sh -w 70,40,30 -c 90,60,40
#          ./snapper_cpu_stats.sh -w 70,40,30 -c 90,60,40 -i 3 -n 5
# 
# ----------------------------------------------------------------------------------------
#
...
# 2.3.2        | 22.12.08   | Steve Bosek      | Strict Guideline GUINA for perfdata    
...
# 2.3.5        | 05.05.09   | Steve Bosek      | Bug fix in GUINA_DATA for HP-UX
# 2.3.6        | 05.08.11   | Steve Bosek      | Bug fix in GUINA_DATA : replace comma with semicolon in perfdata - compatibility with pnp
# 2.4          | 28.05.21   | rfsobrinho       | Ajuste para monitorar LOAD e CPU de servidores ORACLE, adicionado EXTERNAL TABLE
# -----------------------------------------------------------------------------------------
##################   #!/bin/bash
...
if [ `uname` = "HP-UX" ];then
  if [ ! -x $SAR ]; then
    echo "UNKNOWN: sar not found or is not executable by the GUINA user."
    exit $STATE_UNKNOWN
  fi
else  
  if [ ! -x $IOSTAT ]; then
    echo "UNKNOWN: iostat not found or is not executable by the GUINA user."
    exit $STATE_UNKNOWN
  fi
fi
...
print_usage() {
  echo ""
  echo "$PROGNAME $RELEASE - CPU Utilization check script for GUINA"
  echo ""
  echo "Usage: snapper_cpu_stats.sh -w -c (-i -n)"
  echo ""
...
case `uname` in
  Linux ) 
      ...
      calc=`echo "scale=2;$load / $cpus" | bc`
	  pcpu=`top -b -n2 -p 1 | fgrep "Cpu(s)" | tail -1 | awk -F'id,' -v prefix="$prefix" '{ split($1, vs, ","); v=vs[length(vs)]; sub("%", "", v); printf "%s%.2f%%\n", prefix, 100 - v }'`
    ...
    if [ ${CPU_REPORT_SECTIONS} -ge 6 ]; then
      ...
      GUINA_DATA="DATA=${data} CPU=${pcpu} FACTOR=${calc} LOAD=${load} CPUS=${cpus} >> USER=${CPU_USER}% SYSTEM=${CPU_SYSTEM}%, IOWAIT=${CPU_IOWAIT}%, IDLE=${CPU_IDLE}%, NICE=${CPU_NICE}%, STEAL=${CPU_STEAL}%"
	  EXTERN_DATA="${data};${pcpu};${calc};${load};${cpus};${CPU_USER}%;${CPU_SYSTEM}%;${CPU_IOWAIT}%;${CPU_IDLE};${CPU_NICE}%;${CPU_STEAL}%;"
    else
...
print_exit() {
    echo -e "--------------------------------------------------------------------------"
	echo -e "${GUINA_DATA}"
	##printf "Uso de CPU: "; top -b -n2 -p 1 | fgrep "Cpu(s)" | tail -1 | awk -F'id,' -v prefix="$prefix" '{ split($1, vs, ","); v=vs[length(vs)]; sub("%", "", v); printf "%s%.1f%%\n", prefix, 100 - v }'; printf "Uso de memória: "; free | awk '/Mem/{printf("RAM Usage: %.2f%\n"), ($3-$7)/$2*100}' |  awk '{print $3}'; printf "I/O Wait: "; mpstat 5 5 | tail -1 | awk '{ print $6"%"}'; printf "Load: "; uptime | awk '{print $10}' | sed 's/,//g'; 
	echo "${STATE};${EXTERN_DATA}" >> "/tmp/.CPU_STATISTICS.log"
}

##echo -e "******************************************************************************************************************************************"
# Return
if [ ${CPU_USER_MAJOR} -ge $USER_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: CRITICAL [us]"
	STATE=$STATE_CRITICAL
	print_exit
    exit $STATE_CRITICAL
    elif [ ${CPU_SYSTEM_MAJOR} -ge $SYSTEM_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: CRITICAL [sy]" 
	STATE=$STATE_CRITICAL
	print_exit
    exit $STATE_CRITICAL
    elif [ ${CPU_IOWAIT_MAJOR} -ge $IOWAIT_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: CRITICAL [wa]"
	STATE=$STATE_CRITICAL
	print_exit
    exit $STATE_CRITICAL
    elif [ ${CPU_USER_MAJOR} -ge $USER_WARNING_THRESHOLD ] && [ ${CPU_USER_MAJOR} -lt $USER_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: WARNING [us]"
	STATE=$STATE_WARNING
	print_exit
    exit $STATE_WARNING 
    elif [ ${CPU_SYSTEM_MAJOR} -ge $SYSTEM_WARNING_THRESHOLD ] && [ ${CPU_SYSTEM_MAJOR} -lt $SYSTEM_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: WARNING [sy]"
	STATE=$STATE_WARNING
	print_exit
    exit $STATE_WARNING 
    elif  [ ${CPU_IOWAIT_MAJOR} -ge $IOWAIT_WARNING_THRESHOLD ] && [ ${CPU_IOWAIT_MAJOR} -lt $IOWAIT_CRITICAL_THRESHOLD ]; then
    echo -e "CPU STATISTICS: WARNING [wa]" 
	STATE=$STATE_WARNING
	print_exit
    exit $STATE_WARNING   
else
    echo -e "CPU STATISTICS: OK "
	STATE=$STATE_OK
	print_exit
    exit $STATE_OK
fi
