...
BEGIN
IF PV_VERSION = 12 THEN   -- In below cursor, I am using IS_RESOLVED_ADAPTIVE_PLAN for 12c.
...
FV_SQL  :=
 q'[select ty_gash(gash.Inst_id,gash.session_id,gash.sql_id,gash.sql_child_number, substr(sql_Text,1,100),decode(sql_plan_hash_value,0,null,sql_plan_hash_value),
    round(decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,0,100*(IO_CELL_OFFLOAD_ELIGIBLE_BYTES-IO_INTERCONNECT_BYTES)
     /decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,1,IO_CELL_OFFLOAD_ELIGIBLE_BYTES))),
     substr(nvl2(sql_profile,'SProf',null)||nvl2(sql_plan_baseline,'SBase',null)||nvl2(sql_patch,'SPatc',null)||nvl2( decode(IS_RESOLVED_ADAPTIVE_PLAN,'Y','Y'),'ADapt',null),1,9),
      pga_allocated, 
...
FV_SQL  := q'[select ty_gash(gash.Inst_id,gash.session_id,gash.sql_id,gash.sql_child_number, substr(sql_Text,1,100),decode(sql_plan_hash_value,0,null,sql_plan_hash_value),
    round(decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,0,100*(IO_CELL_OFFLOAD_ELIGIBLE_BYTES-IO_INTERCONNECT_BYTES)
     /decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,1,IO_CELL_OFFLOAD_ELIGIBLE_BYTES))),
     substr(nvl2(sql_profile,'SProf',null)||nvl2(sql_plan_baseline,'SBase',null)||nvl2(sql_patch,'SPatc',null),1,9),
     pga_allocated,
     temp_space_allocated,
                                case when session_state = 'WAITING' then gash.event else 'ON CPU' end,
                                case when session_state = 'WAITING' then gash.wait_class else 'ON CPU' end,
...
pv_top_sql.delete;
 ...
 FOR II IN ( SELECT  PLANHASH, PCT,SQL_ID||'('||SQL_CHILD||')' SQL_ID, OFFLOADPCT,PLANCNTRL,  pgamb,  tmpmb  FROM (
             SELECT  SQL_ID,SQL_CHILD, PLANHASH, OFFLOADPCT, PLANCNTRL, ROUND(SQLTIME/TOTTIME * 100,1) PCT, PGAMB, TMPMB FROM
               (SELECT SQL_ID,SQL_CHILD, PLANHASH, max(OFFLOADPCT) OFFLOADPCT,min(PLANCNTRL) PLANCNTRL, max(pgamb) pgamb, max(tmpmb) tmpmb,  SUM(TIME_WAITED) SQLTIME FROM  TABLE(FV_ONLY_GASH)
...
SELECT metric_id, METRIC_NAME, LISTAGG(VALUE, ', ') WITHIN GROUP (ORDER BY METRIC_name) "V"
  FROM (
    SELECT
        ...
        FROM (
SELECT metric_id, METRIC_NAME, INST_ID||':'||ROUND(VALUE,1) VALUE FROM GV$SYSMETRIC WHERE GROUP_ID = 2 and metric_id in ( nvl(p_sysmetric_id1,2144),nvl(p_sysmetric_id2,2008),nvl(p_sysmetric_id3,2010) )
            )
)
GROUP BY metric_id, METRIC_NAME
order by Metric_name
)
order by decode (metric_id ,   nvl(p_sysmetric_id1,2144), 1,
                               nvl(p_sysmetric_id2,2008), 2,
                               NVL(P_SYSMETRIC_ID3,2010), 3)
);
--------------------------
end;
