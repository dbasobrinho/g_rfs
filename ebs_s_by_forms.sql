...
-- | Data       : 15/12/2015                                                    |
-- | Exemplo    : @ebs_s_by_forms                                                           |
-- | Arquivo    : ebs_s_by_forms.sql                                                         |
...
PROMPT
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : ALL Sessions by Forms EBS           +-+-+-+-+-+-+-+-+-+-+   |
...
select  
 s.module as module
,s.status, s.sid || ',' || s.serial#|| case when s.inst_id is not null then ',@' || s.inst_id end  as "SID/SERIAL"
...
Where s.paddr       = p.addr    (+)
  ...
  and UPPER(s.module) like UPPER('%&Enter_Forms_Name%')
order by s.status, decode(s.username,'SYS',to_number(s.inst_id||50000000),s.inst_id) , case when instr(SLAVE,',,@') >0 then (substr(SLAVE,3,2)||'1') when instr(SLAVE,'@') >0 then  (decode(upper(s.WAIT_CLASS),'IDLE',2,1)||substr(SLAVE,3,2)||'2') else null end, 
