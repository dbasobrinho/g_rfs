...
PROMPT
PROMPT +-----------------------------------------------------------------------------+
PROMPT | HISTORICO UTILIZACAO DA UNDO POR DIA / HRS [DBA_HIST_UNDOSTAT]              |
PROMPT +-----------------------------------------------------------------------------+
PROMPT
ACCEPT DATA_INI CHAR PROMPT 'DATA (DD/MM/YYYY) = '
...
select substr(time, 1, 11) time, sum(used_GB) used_GB, sum(TUNED_UNDORETENTION) TUNED_UNDORETENTION
  from (select to_char(begin_time, 'yyyymmdd hh24') time,
         ...
         where trunc(begin_time) = to_date('&DATA_INI', 'dd/mm/yyyy') 
