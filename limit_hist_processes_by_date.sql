...
PROMPT
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : HISTORICAL LIMIT PROCESSES          +-+-+-+-+-+-+-+-+-+-+   |
...
col RESOURCE_NAME  format a20          HEADING 'RESOURCE|NAME'  JUSTIFY CENTER 
...
col INSTANCE_NUMBER   format 999    HEADING 'INSTANCE|NUMBER'         JUSTIFY CENTER
SET COLSEP '|' 

	COL SNAPSHOTTIME FOR A20
	COL RESOURCE_NAME FOR A20
	SELECT S.SNAP_ID,s.INSTANCE_NUMBER, TO_CHAR(S.END_INTERVAL_TIME, 'DD-MON HH24:MI:SS') SNAPSHOTTIME,RESOURCE_NAME,
	CURRENT_UTILIZATION,(CURRENT_UTILIZATION*100/LIMIT_VALUE) PERC,
	MAX_UTILIZATION,LIMIT_VALUE 
	FROM DBA_HIST_SNAPSHOT S, DBA_HIST_RESOURCE_LIMIT B
	WHERE S.SNAP_ID = B.SNAP_ID AND upper(RESOURCE_NAME) IN ('PROCESSES') 
	AND  S.END_INTERVAL_TIME >= trunc(sysdate - &days)
	--and CURRENT_UTILIZATION > 2500
