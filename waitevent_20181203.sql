...
COL "DETALHE CHAMADA"  FORMAT a35 Head "ORIGEM:DETALHE|CHAMADA|Mch->Usr->Pg->Call->CMD->Obj->LDT"  JUSTIFY center
...
PROMPT +------------------------------------------------------------------------+
/* FURUSHIMA - WAITEVENT.sql */  with PLSQL_OBJ as
...
SELECT /* FURUSHIMA - WAITEVENT.sql */
               ...
               W.EVENT,
                ...
                REPLACE(DBMS_LOB.SUBSTR(SQLT.SQL_TEXT, 60), CHR(5)) "SQL TEXT",
--             machine||' # '||s.USERNAME||' -> '||OBJETO_PLSQL||' : '|| OBJECT_NAME     as "DETALHE CHAMADA",
...
--      AND SQLT.SQL_TEXT not like '%FURUSHIMA - WAITEVENT.sql%' and SQLT.SQL_TEXT not like  '%S.SID||%||S.SERIAL#%'
---------
ORDER BY
