...
select 'I('||z.isnt||')'|| z.username user_name ,  COUNT(1) total, 
sum(pga_used_mem_mb) pga_used_mem_mb, sum(pga_alloc_mem_mb) pga_alloc_mem_mb, 
sum(pga_freeable_mem_mb) pga_freeable_mem_mb, sum(pga_max_mem_mb) pga_max_mem_mb
from(
...
ORDER BY s.inst_id, s.username, s.osuser) z
GROUP BY  ROLLUP ('I('||z.isnt||')'|| z.username)
;
SET PAGESIZE 14