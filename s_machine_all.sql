...
PROMPT 
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : Sessoes ALL Machine                                         |
PROMPT | Instance : &current_instance                                           |
PROMPT +------------------------------------------------------------------------+
PROMPT ..
PROMPT .
ACCEPT qq_machine CHAR PROMPT 'MACHINE   = '
PROMPT .
SET ECHO        OFF
SET FEEDBACK    6
SET HEADING     ON
...
,case when e.qcsid is not null then e.qcsid || ',' || e.qcserial#|| case when e.inst_id is not null then ',@' || e.inst_id end end  as "SLAVE"
...
,	 to_char(s.seconds_in_wait) as sc_wait, s.status
from gv$session s
,	 gv$process p 
,    gv$px_session e
Where s.paddr       = p.addr    (+)
  ...
  and upper(s.machine) like upper('%&qq_machine%')  
  order by s.status asc, s.inst_id,s.seconds_in_wait desc, e.qcsid || ',' || e.qcserial#, s.username, s.program
/
SET FEEDBACK on
CLEAR BREAKS
CLEAR COLUMNS
