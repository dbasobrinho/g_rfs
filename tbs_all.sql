...
COLUMN ts_size     FORMAT 9,999,999,999,999  HEADING 'TBS Size (MB)'
...
COLUMN pct_used_extend    FORMAT 999                HEADING 'Pct. Extend'
...
SELECT
  ...
  , NVL((a.bytes - NVL(f.bytes, 0)) / a.bytes_extend * 100, 0) pct_used_extend
FROM 
    sys.dba_tablespaces d
  , ( select tablespace_name, sum(bytes) bytes, sum(greatest(maxbytes, bytes)) bytes_extend
      from dba_data_files
...
SELECT
  ...
  , NVL(t.bytes / a.bytes_extend * 100, 0)  pct_used_extend
FROM
    sys.dba_tablespaces d
  , ( select tablespace_name, sum(bytes) bytes, sum(greatest(maxbytes, bytes)) bytes_extend
      from dba_temp_files
