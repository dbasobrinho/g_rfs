-- ASM free space according to the redundancy level of the disks.
SET SERVEROUTPUT ON
SET LINES 155
SET PAGES 0
SET TRIMSPOOL ON
...
-- Set up headings
   ...
   IF dg.free_mb > v_required_free_mb THEN v_enuf_free := TRUE; END IF;
    ...
    END IF;
      ...
      DBMS_OUTPUT.PUT('|'||TO_CHAR(v_max_total_mb/1024,'999,999,999'));
      DBMS_OUTPUT.PUT('|'||TO_CHAR(dg.total_mb/1024,'999,999,999,999'));
      DBMS_OUTPUT.PUT('|'||TO_CHAR((dg.total_mb - dg.free_mb)/1024,'999,999,999,999'));
      DBMS_OUTPUT.PUT('|'||TO_CHAR(dg.free_mb/1024,'999,999,999,999'));
      DBMS_OUTPUT.PUT('|'||TO_CHAR(ROUND(v_required_free_mb/1024),'999,999,999,999'));
      DBMS_OUTPUT.PUT('|'||TO_CHAR(ROUND(v_usable_mb/1024),'999,999,999,999'));
