-- |----------------------------------------------------------------------------|
-- | DATABASE : Oracle                                                          |
-- | FILE     : sess_user_sessions_by_user.sql                                  |
...
PROMPT 
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : User Sessions Summary Report by user                        |
...
COLUMN num_user_sess     FORMAT 99999           JUSTIFY r HEADING 'Number of Logins'
COLUMN IDLE_TIME         FORMAT 9,999,999,999   JUSTIFY r HEADING 'Limit Time'
COLUMN SESSIONS_PER_USER FORMAT 9,999,999,999   JUSTIFY r HEADING 'Limit Sess'
...
select z.username, z.num_user_sess, z.count_a, z.count_i, lpad(z.username, 20) username
,(SELECT to_number(trim(limit)) as a FROM DBA_profiles WHERE  PROFILE in (SELECT profile FROM DBA_USERS WHERE USERNAME = z.username) AND resource_name = 'IDLE_TIME' and limit not in ('DEFAULT', 'UNLIMITED')) as "Limit Time"
,(SELECT to_number(trim(limit)) as b FROM DBA_profiles WHERE  PROFILE in (SELECT profile FROM DBA_USERS WHERE USERNAME = z.username) AND resource_name = 'SESSIONS_PER_USER' and limit not in ('DEFAULT', 'UNLIMITED')) as "Limit Sess"
,lpad(TRUNC(((z.num_user_sess * 100) / 
(SELECT to_number(trim(limit)) as b FROM DBA_profiles WHERE  PROFILE in (SELECT profile FROM DBA_USERS WHERE USERNAME = z.username) AND resource_name = 'SESSIONS_PER_USER' and limit not in ('DEFAULT', 'UNLIMITED'))))  || '%', 19) pct_utl
from (
SELECT
    nvl(sess.username, '[B.G. Process]') username
  , count(1) num_user_sess
  --, count(sid) num_user_proc
...
GROUP BY 
  ...
  , act.count
  , inact.count
  order by nvl2(sess.username, 1,2) , 2 desc )z
/
