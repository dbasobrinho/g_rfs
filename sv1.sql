...
col slave        format a13  HEADING 'SLAVE/W_CLASS'
col opid         format a04
...
Where s.paddr       = p.addr    (+)
  ...
  order by decode(upper(s.WAIT_CLASS),'IDLE',1,0), decode(s.username,'SYS',1,0), s.inst_id,s.seconds_in_wait desc, e.qcsid || ',' || e.qcserial#, s.username, s.program
/
SET FEEDBACK on
--SET COLSEP ' '
