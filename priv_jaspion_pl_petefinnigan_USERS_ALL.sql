...
-- -----------------------------------------------------------------------------
-- Maintainer  : Pete Finnigan (http://www.petefinnigan.com)   petefinnigan.com)
           ...
           select
             ...
             from (select 'ROLE' typ,
                   ...
                   select 'TABLE' typ,
                          ...
                          '--' pwd
                     from dba_tab_privs
                    where grantee = cp_grantee and 'N' = 'Y'
                   union
                   select 'COLUMN' typ,
                          grantee grantee,
                          privilege priv,
                          ...
                          '--' pwd
                     from dba_col_privs
                    where grantee = cp_grantee and 'N' = 'Y') a
         order by case
...
begin
FOR C1 IN (select username from dba_users where oracle_maintained <> 'Y' ORDER BY 1)
LOOP
   lv_file_or_screen := upper ('FILE');

   if lv_file_or_screen = 'F' then
      open_file (c1.username||'.txt', 'TMP');
   end if;

   write_op (
      'User => ' || upper (c1.username) || ' has been granted the following privileges');
   write_op ('====================================================================');
   get_privs (upper (c1.username), lv_tabs);
