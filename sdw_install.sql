...
PROMPT |                                                                                                                                      |   
PROMPT +--------------------------------------------------------------------------------------------------------------------------------------+                                           
PROMPT |                                      _____ __   _ _______ _______ _______                                                            |
PROMPT |                                        |   | \  | |______    |    |_____| |      |                                                   |
PROMPT |                                      __|__ |  \_| ______|    |    |     | |_____ |_____                                              |
PROMPT |                                                                                                                                      |
PROMPT +--------------------------------------------------------------------------------------------------------------------------------------+
...
drop type ash_aas_ntt;
drop type ash_aas_ot;
drop type ash_graph_ntt;
drop type ash_graph_ot;
...
create or replace package moats as
   ...
   function top (
			...
			p_clear_screen    in integer default 0
   ...
   ----------------------------------------------------------------------------
    ...
    begin
     return moats_output_ntt(
   			moats_output_ot(''), moats_output_ot(''), moats_output_ot(''), moats_output_ot(''), moats_output_ot(''), moats_output_ot(''),
   			moats_output_ot('=========================================================================================================='),
   			...
   			moats_output_ot(''),
			moats_output_ot(''),
   			moats_output_ot('>> . . . O GUINA NAO TINHA DO, SE REGIR BUMMMM! VIRRA PO . . . <<'), 
   			moats_output_ot(''),
   			moats_output_ot(''),
   			moats_output_ot(''),
   			moats_output_ot(''),
   			...
   			moats_output_ot(''),			
   			moats_output_ot(''),			
   			moats_output_ot('				              ______ _ ___                        '),					
   			moats_output_ot('				             |_  / _` / __| +-+-+-+-+-+-+-+-+-+-+ '),					
   			moats_output_ot('				  _   _   _   / / (_| \__ \ |r|f|s|o|b|r|i|n|h|o| '),					
   			moats_output_ot('				 (_) (_) (_) /___\__,_|___/ +-+-+-+-+-+-+-+-+-+-+ ')			
   			);	
    end banner;
...
--+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------+                                                                                                                                   ..';
--v_header := v_header||' +--------------------------------------------------------------------------------------------------------------------------------------+ ';
 --v_header := v_header||' |  _____   ______ _______ _______        _______               ______  _______ _______ _     _ ______   _____  _______  ______ ______  | ';  
 --v_header := v_header||' | |     | |_____/ |_____| |       |      |______      ___      |     \ |_____| |______ |_____| |_____] |     | |_____| |_____/ |     \ | ';   
 --v_header := v_header||' | |_____| |    \_ |     | |_____  |_____ |______               |_____/ |     | ______| |     | |_____] |_____| |     | |    \_ |_____/ | ';   
--v_header := v_header||' |                                                                                                                                      | ';  
--v_header := v_header||' +--------------------------------------------------------------------------------------------------------------------------------------+ ';
--v_header := v_header||' |                                             _______ _____ _    _ _____ _______                         +-+-+-+-+-+-+-+-+-+-+         | ';
--v_header := v_header||' |                                                |      |    \  /    |      |                            |r|f|s|o|b|r|i|n|h|o|         | ';
--v_header := v_header||' |                                                |    __|__   \/   __|__    |                            +-+-+-+-+-+-+-+-+-+-+         | ';
--v_header := v_header||' |                                                                                                                                      | ';
--v_header := v_header||' +--------------------------------------------------------------------------------------------------------------------------------------+ ';
--v_header := v_header||' | Author : Adrian Billington                                                                                                           | ';  
--v_header := v_header||' | Version: V1.0                                                                                                                        | ';  
--v_header := v_header||' |                                                                                                              +-+-+-+-+-+-+-+-+-+-+   | ';  
--v_header := v_header||' | ref    : https://github.com/dbsid/moats_rac                                                                  |r|f|s|o|b|r|i|n|h|o|   | ';  
--v_header := v_header||' |                                                                                                              +-+-+-+-+-+-+-+-+-+-+   | ';          
--v_header := v_header||' +--------------------------------------------------------------------------------------------------------------------------------------+ ';
--v_header := v_header||' ..	                                                                                                                                   ..';
	  
      v_rows.extend(1); v_header := null;
      v_rows(v_rows.last)  := moats_output_ot(rpad(''||v_header||'+', 172,'-')|| '+');	  
      -->>
      v_rows.extend(1);v_header := '----------------+ |  _____   ______ _______ _______        _______               ______  _______ _______ _     _ ______   _____  _______  ______ ______  | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ | |     | |_____/ |_____| |       |      |______      ___      |     \ |_____| |______ |_____| |_____] |     | |_____| |_____/ |     \ | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ | |_____| |    \_ |     | |_____  |_____ |______               |_____/ |     | ______| |     | |_____] |_____| |     | |    \_ |_____/ | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ |                                                                                                                                      | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :=null;
      v_rows(v_rows.last)  := moats_output_ot(rpad(''||v_header||'+', 172,'-')|| '+');	
      v_rows.extend(1); v_header :='----------------+ |                                             _______ _____ _    _ _____ _______                                                       | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ |                                                |      |    \  /    |      |                            +-+-+-+-+-+-+-+-+-+-+         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');	
      v_rows.extend(1); v_header :='----------------+ |                                                |    __|__   \/   __|__    |                            |r|f|s|o|b|r|i|n|h|o|         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');
      v_rows.extend(1); v_header :='----------------+ |                                                                                                        +-+-+-+-+-+-+-+-+-+-+         | ';
      v_rows(v_rows.last)  := moats_output_ot(rpad('+'||v_header||'+', 172,'-') || '+');
      -->>
      v_rows.extend(1); v_header :=null;
      v_rows(v_rows.last)  :=  moats_output_ot(rpad(''||v_header||'+', 172,'-')|| '+');	 
      --v_rows.extend(1); v_header :=  '----------------+ '|| rpad('| Current Time: ' ||to_char(sysdate, 'DD/MM/YYYY hh24:mi:ss'), 36) ||rpad('| ASH History:  ' || v_hstr, 27) || '|';
      --v_rows(v_rows.last)  := moats_output_ot(rpad(''||v_header||'+', 172,'-') || '+');
      --v_rows.extend(1); v_header := null;
      --v_rows(v_rows.last)  := moats_output_ot(rpad(''||v_header||'+', 172,'-') || '+');	  
      --v_rows.extend(1); v_header := null;
...
--      v_header := '+ Database: ' || rpad(v_dbname, 8) || rpad('| Current Time: ' ||to_char(sysdate, 'DD-Mon hh24:mi:ss'), 36) ||rpad('| ASH History:  ' || v_hstr, 27) || '|';
--      -- Set the instance summary output...
      -- ----------------------------------

     --v_rows.extend(3);
	  --/rfsobrinho 21/02/2010
      --v_rows(1) := moats_output_ot(v_header);
	  v_rows.extend(1);
      V_ROWS(V_ROWS.LAST):= MOATS_OUTPUT_OT(RPAD('+ DATABASE: ' ||  RPAD(V_DBNAME, 8)            || ' |' || RPAD(' ACTIVITY STATISTICS PER SECOND',33) || '|' ||
	  ...
	  v_rows.extend(1);
      v_rows(v_rows.last) := moats_output_ot(v_headstr);
   ...
   begin
      ...
      pipe row (moats_output_ot('Aguarde: buscando dados para a primeira atualização...'));
		--pipe row (moats_output_ot('>> . . . O GUINA NAO TINHA DO, SE REGIR BUMMMM! VIRRA PO . . . <<'));
