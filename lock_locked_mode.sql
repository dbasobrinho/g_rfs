...
SELECT s.sid || ',' || s.serial#|| case when s.inst_id is not null then ',@' || s.inst_id end as sess
	...
	--  , decode(s.state,'WAITING','Waiting '||seconds_in_wait,'Working '||wait_time / 100)||' '||event state
      ...
      ,decode(b.locked_mode, 0, 'None', 
                            1, 'Null (NULL) ()', 
                            2, 'Row-S (SS)   (Transação possui linhas ""locada"s" exclusivamente, mas ainda não as alterou)', 
                            3, 'Row-X (SX)   (Transação possui linhas "locadas" exclusivamente e já as alterou)', 
                            4, 'Share (S)    (Instrução "LOCK TABLE table IN SHARE MODE")', 
                            5, 'S/Row-X (SSX)(Somente uma transação pode obtê-lo por vez)', 
                            6, 'Exclusive (X)(Permite apenas que as outras sessões acessem a tabela através de instruções SELECTs) ', 
