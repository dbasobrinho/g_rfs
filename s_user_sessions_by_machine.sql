-- |----------------------------------------------------------------------------|
-- | DATABASE : Oracle                                                          |
-- | FILE     : sess_user_sessions_by_machine.sql                               |
...
PROMPT 
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : User Sessions Summary Report by machine                     |
...
FROM 
    v$session sess
  , (SELECT    count(*) count, nvl(machine, '/') machine
     FROM      v$session
     WHERE     status = 'ACTIVE'
...
WHERE
         nvl(sess.machine, '/') = act.machine   (+)
     and nvl(sess.machine, '/') = inact.machine (+)
GROUP BY 
   sess.machine
  , act.count
  , inact.count
  order by count_i desc, count_a asc, machine
/
