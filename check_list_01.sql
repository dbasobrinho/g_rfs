...
PROMPT ****************** SESSÕS ATIVAS DO BANCO DE DADOS ******************
prompt
...
PROMPT ****************** INSTÂCIAS REGISTRADAS NO ORATAB ******************
prompt
!cat /etc/oratab
...
PROMPT ****************** ARCHIVELOG ÁEA DE DB_RECOVER ******************
prompt
...
TTITLE LEFT SKIP 1 "ASM DISK GROUPS" SKIP 2
	...
	SELECT 	group_number  "GROUP",
			...
			free_mb       "FREE_MB" ,
			DECODE(100 - ceil(FREE_MB/TOTAL_MB*100), NULL, 100, 100 - ceil(FREE_MB/TOTAL_MB*100)) "% USADO"
	FROM    v$asm_diskgroup, v$instance where name like 'FRA';
TTITLE OFF
PROMPT
prompt


prompt ***************** ARCHIVELOG FILESYSTEM, COPIAR E COLAR NO TERMINAL *******************
prompt
col cmd for a50
select '!df -h '||DESTINATION cmd from V$ARCHIVE_DEST where DESTINATION is not null and DESTINATION not like '%+%' and DESTINATION not like '%USE_DB_%';
...
prompt ****************** TESTE DE CONEXAO NA INSTANCIA ******************
PROMPT tvtspi
prompt
conn tvtspi/tvtspi123
prompt
prompt
