-- |----------------------------------------------------------------------------|
-- | Objetivo   : CHILD CURSOR MAIOR QUE 10                                     |
-- | Criado por : Roberto Fernandes Sobrinho                                    |
-- | Data       : 05/10/2018                                                    |
-- | Exemplo    : @child_cursor_sqlid_by_greater 10                             |
-- | Arquivo    : child_cursor_sqlid_by_greater.sql                             |
-- | Modificacao:                                                               |
-- +----------------------------------------------------------------------------+
SET TERMOUT OFF;
...
COLUMN reason_not_shared FORMAT A50    head "REASON_NOT_SHARED"
COLUMN scma              FORMAT A30    head "PARSING|SCHEMA"
...
COLUMN TEXT30           FORMAT A30     head "SQL_TEXT|30"
COLUMN cnt              FORMAT 999999  head "LOADED|VERSIONS"

-------ACCEPT sssql_txt  number default 1  PROMPT 'COUNT CHILD >  = '

select inst_id, parsing_schema_name scma, reason_not_shared, sql_id, count(*) cnt, substr(SQL_TEXT,1,30)  TEXT30
...
group by inst_id, parsing_schema_name, reason_not_shared, sql_id, substr(SQL_TEXT,1,30)
having count(*) >= &1
