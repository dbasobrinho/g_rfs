...
set verify off  
set feedback ON
COLUMN OWNER                FORMAT A13              HEADING "OWNER|-"             JUSTIFY CENTER
COLUMN SEGMENT_NAME         FORMAT A40              HEADING "SEGMENT|NAME"             JUSTIFY CENTER
COLUMN PART_NAME            FORMAT A15              HEADING "PARTITION|NAME"             JUSTIFY CENTER
COLUMN SIZE_POS             FORMAT A08              HEADING "SIZE|POSITION"             JUSTIFY CENTER
COLUMN IO_POS               FORMAT A08              HEADING "IO|POSITION"             JUSTIFY CENTER
COLUMN UNUSED_MB            FORMAT A15     HEADING "UNUSED|(MB)"             JUSTIFY CENTER
COLUMN "QUALITY_%"          FORMAT 999999             HEADING "QUALITY|(%)"             JUSTIFY CENTER
COLUMN SEG_GROSS_MB         FORMAT A15     HEADING "SEG GROSS|(MB)"             JUSTIFY CENTER
COLUMN SEG_NET_MB           FORMAT A15     HEADING "SEG LOGICAL|(MB)"             JUSTIFY CENTER
COLUMN DATA_GROSS_MB        FORMAT A15     HEADING "DATA GROSS|(MB)"             JUSTIFY CENTER
COLUMN DATA_NET_MB          FORMAT A15              HEADING "DATA LOGICAL|(MB)"             JUSTIFY CENTER


COMPUTE sum LABEL "Grand Total: " OF total_UNUSED_MB "UNUSED|(MB)" ON report


SELECT NULL OWNER, NULL SEGMENT_NAME, NULL PART_NAME, NULL SIZE_POS, NULL IO_POS, NULL UNUSED_MB, NULL "QUALITY_%", 
...
( SELECT /*+ MATERIALIZE */
    ...
    S.SIZE_POS,
    S.IO_POS,
    DECODE(T.COMPRESSION, 'ENABLED', 'YES', 'NO') COMPRESSION
  FROM
    BASIS_INFO TI,
    SEGMENT_SELECTION S,
    DBA_TABLES T
  ...
  ( SELECT /*+ MATERIALIZE */
    ...
    FROM
      ...
      DBA_TABLES T
    WHERE
      PT.OWNER LIKE TI.TABLE_OWNER AND
      PT.TABLE_NAME LIKE TI.TABLE_NAME AND
      PT.OWNER = S.OWNER AND
      PT.TABLE_NAME = S.SEGMENT_NAME AND
      PT.DEF_TABLESPACE_NAME LIKE TI.TABLESPACE_NAME AND
  ...
  ( SELECT  OWNER,
    ...
    ( SELECT
        ...
        DECODE(CLUSTER_TABLE, 'YES', DTS.BLOCKS * (8000 - 23 * TS.INI_TRANS) * 
          (1 - TS.PCT_FREE / 100) / 1024 / 1024, 
		  (S.BYTES / 1024 / 1024) * (8000 - 23 * TS.INI_TRANS) * (1 - TS.PCT_FREE / 100) / rfs.BLOCK_SIZE) SEG_NET_MB,
        ...
        ' ' INDEX_NAME
      FROM 
        TABLE_STORAGE TS,          DBA_TABLESPACES rfs,
        TABLE_MODIFICATIONS TM,
        DBA_TAB_STATISTICS DTS,
        TABLE_COLUMNS DTC,
        SEGMENTS S,
        BASIS_INFO BI
      WHERE rfs.TABLESPACE_NAME = TS.TABLESPACE_NAME and
