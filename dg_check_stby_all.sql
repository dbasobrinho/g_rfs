...
COLUMN REPLICACAO_DG          FORMAT a28          
COLUMN STANDBY_LAST_RECEIVED  FORMAT 9999999    HEAD 'Last Standby|Seq Received'  
COLUMN STANDBY_LAST_APPLIED   FORMAT 9999999    HEAD 'Last Standby|Seq Applied'    
COLUMN STANDBY_DT_LAST_APP    FORMAT a20        HEAD 'Last Date|Applied' 
COLUMN STANDBY_LAG            FORMAT 999999     HEAD 'Lag|Applied'
    

select (select value from v$parameter a where name = 'log_archive_config') REPLICACAO_DG,
       b. last_seq   STANDBY_LAST_RECEIVED ,  --Last Standby Seq Applied
       a.applied_seq STANDBY_LAST_APPLIED  , --Last Standby Seq Applied
       TO_CHAR(a.last_app_timestamp,'DD/MM/YYYY HH24:MI:SS') STANDBY_DT_LAST_APP, 
       b.last_seq - a.applied_seq STANDBY_LAG , b.inst_id, b.thread#
from   (select thread#, 
               (inst_id) as inst_id,
        ...
        where  applied = 'YES' 
        group  by thread#, inst_id) a, 
       (select thread#, 
               (inst_id) as inst_id,
               max (sequence#) last_seq 
        from   gv$archived_log 
        group  by thread#, inst_id) b a
