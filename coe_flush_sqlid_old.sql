...
PRO *** before flush ***
SELECT inst_id, loaded_versions, invalidations, address, hash_value hash_valuee
...
BEGIN
 ...
 LOOP SYS.DBMS_SHARED_POOL.PURGE(i.address||','||i.hash_value, 'C'); END LOOP;
END;
/
set echo off
PRO *** after flush ***
SELECT inst_id, loaded_versions, invalidations, address, hash_value hash_valuee
FROM gv$sqlarea WHERE sql_id = '&&sql_id.' ORDER BY 1;
SELECT inst_id, child_number, plan_hash_value hash_valuee, executions, is_shareable
FROM gv$sql WHERE sql_id = '&&sql_id.' ORDER BY 1, 2;
UNDEF sql_id;
SPO OFF;
