...
-- | Data       : 02/04/2018                                                    |
-- | Exemplo    : coe_sql_fudidos_by_date                                       |
-- | Arquivo    : coe_sql_fudidos_by_date.sql                                   |
    ...
    from (select ash.sql_id,
            ...
            from (select sql_id,
                    ...
                    from (select sql_id,
                           ...
                           where sample_time BETWEEN to_date('&&DATA_INI', 'dd/mm/yyyy hh24:mi:ss') and
...
(SELECT sql.sql_id,
         ...
         trunc(avg(OPTIMIZER_COST) * sum(sql.executions_delta)) COST_ADV
    FROM dba_hist_sqlstat sql
    JOIN dba_hist_snapshot ss
      ON sql.snap_id = ss.snap_id
   WHERE ss.begin_interval_time BETWEEN to_date('&&DATA_INI', 'dd/mm/yyyy hh24:mi:ss') and
                                        to_date('&&DATA_FIM', 'dd/mm/yyyy hh24:mi:ss')
