...
PROMPT #####################################################################
PROMPT MONITORA PROGRESSO MVIEW FAST [MLOG$_AU_PAYMENT_MEDIA@AUTORIZADOR]
PROMPT #####################################################################
select distinct DMLTYPE$$,count(*) Registros_mlog from sysau.MLOG$_AU_PAYMENT_MEDIA@AUTORIZADOR group by DMLTYPE$$;
Select distinct inicio as dt_inicio,
  ...
  from (select max(data_checkpoint) INICIO
       ...
       (select count(*) Registros
          from sysau.MLOG$_AU_PAYMENT_MEDIA@AUTORIZADOR),
       (SELECT total_inserts_knstmvr + total_updates_knstmvr +
         ...
         (SELECT 1
           ...
           and currmvname_knstmvr like '%AU_PAYMENT_MEDIA%')
/   
