...
COLUMN bytes       FORMAT 9,999,999,999,999  HEADING 'TBS Size (MB)'
...
SELECT
  ...
  , NVL(SUM(b.bytes)/COUNT(DISTINCT a.file_id||'.'||a.block_id ) -
        ...
        SUM(b.bytes)/COUNT(DISTINCT a.file_id||'.'||a.block_id ))  / (1024 * 1024)   used
  , NVL(SUM(a.bytes)/COUNT(DISTINCT b.file_id ),0)   / (1024 * 1024)                free
