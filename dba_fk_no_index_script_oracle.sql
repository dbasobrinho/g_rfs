set heading off 
select 'Write output to  table bumxxbum_FOREIGN_KEY_EXCEP created in this schema Y/N:' from dual;
...
BEGIN
 begin EXECUTE IMMEDIATE 'DROP TABLE  bumxxbum_foreign_key_excep'; exception when others then null; end;
 begin EXECUTE IMMEDIATE 'CREATE TABLE bumxxbum_foreign_key_excep (owner VARCHAR2(30),constraint_name VARCHAR2(30),status VARCHAR2(8),table_name VARCHAR2(30),foreign_key VARCHAR2(2000))'; exception when others then null; end;
...
IF WRITE_TO_TABLE_Y_N ='Y' or   WRITE_TO_TABLE_Y_N ='y' THEN
     INSERT INTO bumxxbum_foreign_key_excep VALUES (c1_rec.owner,c1_rec.constraint_name,c1_rec.status, c1_rec.table_name,pl_foreign_key);
END IF;
/* sdixon: Changed from EXECute immedaite which generated ORA-984 - from comment HP ID 2327281
      EXECUTE IMMEDIATE 'INSERT INTO bumxxbum_foreign_key_excep VALUES (c1_rec.owner,c1_rec.constraint_name,c1_rec.status, c1_rec.table_name,pl_foreign_key)';
*/
