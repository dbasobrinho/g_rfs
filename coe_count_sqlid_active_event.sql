...
PROMPT
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : SALDO EXTRATO ALELO                 +-+-+-+-+-+-+-+-+-+-+   |
...
col service_name format a20 HEADING 'SERVICE_NAME'
SET COLSEP '|'

select count(1) tot, sql_id, sessionwait, machine
from(
...
decode(s.username,'SYS',50000000,sc_wait), s.machine, s.last_call_et) group by sql_id, sessionwait, sql_id, machine order by 1,2
/
...
Where s.paddr       = p.addr    (+)
  ...
  and s.username is not null
order by decode(s.username,'SYS',to_number(s.inst_id||50000000),s.inst_id) , case when instr(SLAVE,',,@') >0 then (substr(SLAVE,3,2)||'1') when instr(SLAVE,'@') >0 then  (decode(upper(s.WAIT_CLASS),'IDLE',2,1)||substr(SLAVE,3,2)||'2') else null end, 
decode(s.username,'SYS',50000000,sc_wait), s.machine, s.last_call_et) group by sql_id, sessionwait, sql_id order by 1,2
/
SET FEEDBACK on
--SET COLSEP ' '
