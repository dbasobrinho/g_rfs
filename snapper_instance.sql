...
begin
    ...
    if pagesize > 0 then
        output(' ');
        output('-- GUINA NAO TINHA DO, SE REAGIR BUMMM! VIRA PO!!!');
            output('-- SNAPPER V4.31 >> BY TANEL PODER >> ADVANCED ORACLE TROUBLESHOOTING! :)');
        output(' ');
    end if;
...
&_USE_DBMS_LOCK ash_date2 := sysdate;
        ...
        if gather_stats = 1 then
            ...
            if pagesize > 0 and changed_values > 0 then
                output(' ');
                output('--  End of Stats snap '||to_char(c)||', end='||to_char(d2, 'YYYY-MM-DD HH24:MI:SS')||', seconds='||round(get_seconds(d2-d1), 1)||' e ZAS . . .');
            end if;
        ...
        if gather_ash = 1 then
            ...
            if pagesize > 0 then
                output(' ');
                output('--  End of ASH snap '||to_char(c)||', end='||to_char(ash_date2, 'YYYY-MM-DD HH24:MI:SS')||', seconds='||to_char(round((ash_date2-ash_date1) * 86400, 1))||', samples_taken='||g_ash_samples_taken||', AAS='||CASE WHEN g_ash_samples_taken = 0 THEN '(No ASH sampling in begin/end snapshot mode)' ELSE TO_CHAR(ROUND(g_ash.COUNT/NULLIF(g_ash_samples_taken,0),1)) END ||' e ZAS . . .');
